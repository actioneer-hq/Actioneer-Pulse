...
def recluster(db: Session) -> dict:
    """Full pass: back-fill embeddings, then recluster every tenant×lever over the rolling window."""
    ...
    out: dict[str, int] = {}
    tenants = db.scalars(select(CallEmbedding.tenant_id).distinct()).all()
    for tenant_id in tenants:
        for lever in LEVERS:
            out[f"{tenant_id}:{lever}"] = _recluster_lever(db, tenant_id, lever, since)
    db.commit()
    return {"clusters": out}
...
def _backfill_embeddings(db, resolved) -> None:
    ...
    for lever, extract in LEVERS.items():
        ...
        pending: list[tuple[str, str, str]] = []  # (call_id, tenant_id, text)
        for j in db.scalars(select(Judgment)):
            if j.call_id in have:
                continue
            text = extract(j)
            if text and text.strip():
                pending.append((j.call_id, j.tenant_id, text.strip()))
        for i in range(0, len(pending), _EMBED_BATCH):
            batch = pending[i : i + _EMBED_BATCH]
            try:
                vecs = gateway.embed(resolved, [t for _, _, t in batch])
            except Exception as e:  # noqa: BLE001 — embedding is best-effort; skip the batch
                log.warning("embedding batch failed (%s): %s", lever, e)
                continue
            for (call_id, tenant_id, _), vec in zip(batch, vecs):
                db.add(CallEmbedding(call_id=call_id, tenant_id=tenant_id, field=lever,
                                     embedding=vec, model=resolved.model))
        db.flush()


def _recluster_lever(db, tenant_id: str, lever: str, since: datetime) -> int:
    rows = db.execute(
        select(CallEmbedding.call_id, CallEmbedding.embedding)
        .join(Call, Call.id == CallEmbedding.call_id)
        .where(CallEmbedding.tenant_id == tenant_id, CallEmbedding.field == lever,
               func.coalesce(Call.started_at, Call.created_at) >= since)
    ).all()
    # always rewrite this tenant/lever
    db.execute(delete(CallCluster).where(CallCluster.tenant_id == tenant_id, CallCluster.lever == lever))
    db.execute(delete(Cluster).where(Cluster.tenant_id == tenant_id, Cluster.lever == lever))
    ...
    for call_id, key, (x, y) in zip(call_ids, labels, coords):
        db.add(CallCluster(call_id=call_id, tenant_id=tenant_id, lever=lever,
    ...
    texts = dict(db.execute(  # call_id -> prose, for labelling exemplars
        select(Judgment.call_id, _text_col(lever)).where(Judgment.tenant_id == tenant_id)
    ).all()) if members else {}
    for key, ids in members.items():
        label = _label_cluster(lever, [texts.get(cid) for cid in ids[:5] if texts.get(cid)])
        db.add(Cluster(tenant_id=tenant_id, lever=lever, cluster_key=key,
                       label=label, size=len(ids)))
    db.flush()
    return len(members)
