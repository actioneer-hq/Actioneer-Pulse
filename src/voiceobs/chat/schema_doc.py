"""The system-prompt pieces for the global-chat SQL agent: the curated schema the model is allowed
to query, and a compact Postgres/pgvector SQL skill. Assembled by `build_system_prompt`.

The schema here mirrors the read-only views in `db/agent_views.py` (the agent's entire visible
database). Enum values, join keys, and units are spelled out because that is what makes text-to-SQL
reliable. Keep this in sync with AGENT_VIEWS."""

from __future__ import annotations

SCHEMA_DOC = """\
You answer questions about an organization's voice-agent calls by writing ONE read-only SQL query
and calling the `execute_sql` tool. These are the ONLY tables that exist. Join everything on
`call_id` (→ calls.id). All timestamps are UTC.

-- One row per voice call.
calls(
  id, external_call_id, agent_id,          -- agent_id → agents.id
  source, environment,                     -- environment: 'prod' | 'staging' | 'dev'
  engine,                                  -- 'cascade' (STT→LLM→TTS) | 's2s' (speech-to-speech)
  carrier, stt_provider, llm_provider, tts_provider, voice, campaign_id,
  started_at, ended_at, duration_s,        -- duration_s in SECONDS
  status,                                  -- 'complete' | 'ingested' | 'awaiting_media' | 'unsupported' | 'failed'
  terminal_reason, hangup_by,              -- hangup_by: 'agent' | 'remote' | 'error'
  cost_llm, cost_stt, cost_tts, cost_total, cost_currency,
  tokens_in, tokens_out, tokens_cached, stt_seconds, bookmarked, created_at
)
...
events(
  ...
  content_text, content_kind               -- content_kind: 'transcript' | 'llm_raw' | 'llm_spoken'
)

-- One row per conversational turn (exchange). Latencies are in MILLISECONDS.
turns(
  call_id, turn_index, trigger, interrupted, abandoned,
  response_latency_ms,                     -- caller stopped → agent started (voice-to-voice)
  stt_lag_ms, endpointing_ms, llm_ttft_ms, assembly_ms, dispatch_ms, tts_ttfb_ms, playout_ms,
  e2e_latency_ms, language, stt_confidence, tokens_in, tokens_out, finish_reason,
  tts_cancelled, cut_reason,
  caller_transcript, llm_spoken            -- what the caller said / what the agent said
)

-- From-audio speech segments per channel (VAD on the recording).
utterances(
  call_id, channel,                        -- channel: 'caller' | 'agent'
  t_start_s, t_end_s                       -- seconds
)

-- Computed per-call metrics, TALL format (one row per metric name). Aggregate on value_num.
metrics(call_id, name, value_num, value_text, available, reason)

-- LLM post-call analysis, one row per call. Failure fields are non-null only when is_failure = true.
judgments(
  call_id, disposition,                    -- programmatic, e.g. 'connected','no_answer','voicemail'
  status,                                  -- 'ok' | 'skipped' | 'failed'
  sentiment,                               -- 'positive' | 'neutral' | 'negative'
  objective_achieved,                      -- 'yes' | 'no' | 'partial' | 'unknown'
  answered_by, primary_language, secondary_languages,
  script_adherence,                        -- 'high' | 'medium' | 'low'
  escalation_requested, callback_requested, callback_time,
  guardrail_violation, guardrail_violation_points,   -- boolean + JSON array of broken rules
  is_failure,                              -- boolean
  model_fault,                             -- 'none' | 'asr' | 'llm' | 'tts' | 'other'
  model_fault_detail, hallucination, hallucination_detail,
...
SQL_SKILL = """\
SQL guidance (PostgreSQL):
- Read-only. Emit exactly ONE statement, a SELECT (or WITH … SELECT). Always add a LIMIT (≤ 500)
  unless you are aggregating to a single row.
- Dialect: Postgres. Dates: date_trunc('day', started_at), now() - interval '7 days',
  extract(epoch from …). Percentiles: percentile_cont(0.95) WITHIN GROUP (ORDER BY x).
- Trends/correlation without extra tools: regr_slope(y, x), regr_r2(y, x), corr(y, x).
- metrics is TALL: filter by name, aggregate value_num, e.g.
...
citing the numbers you found — don't paste raw tables unless asked.
"""
...
PGVECTOR_SKILL = _PGVECTOR_PREFACE + r"""# pgvector for Semantic Search
