...
A framework subclasses `Calculator` ONLY when its producer's *timing model* differs from
the canonical one; it then overrides an anchor hook (`end_of_speech`, `endpointing_ms`,
`first_token`, `tts_request_start`, `first_audio`). The waterfall arithmetic, the
interval-union `unattributed`, the metrics and the trust report are shared and not meant to
be overridden.

Anchoring: spans are on the call clock, utterances on the audio clock;
audio_time = call_time - t0_offset_s places spans onto audio. t0_offset_s lives on AudioRef,
so it is passed as a kwarg (None = already co-clocked).
...
class Calculator:
    """Generic Trace -> Analysis. Override an anchor hook only for a producer whose timing
    genuinely differs; everything else is shared."""
    ...
    def analyze(
        ...
        "audio never arrived" (an outage) in the trust report — the numbers are identical,
        the reason is not."""
    ...
    # ── anchor hooks — the per-turn timestamps a new framework may retune ──────────
    # Each default is the canonical behavior: an event when the producer emits one, else
    # the reported-attribute reconstruction. Override one only if your producer disagrees.

    def end_of_speech(self, speech: list[Span], offset: float) -> float | None:
        """When the caller stopped talking — the last end across their speech segments."""
        ends = [s.t_end for s in speech if s.t_end is not None]
        return _to_audio(max(ends), offset) if ends else None

    def endpointing_ms(
        self, speech_end: float | None, stt_start: float | None, related: list[Span]
    ) -> float | None:
        """The silence hold: end of speech -> STT starts. Falls back to the producer's own
        reported `endpointing.delay` when there is no SPEECH->STT gap to measure."""
        v = _ms(speech_end, stt_start)
        return v if v is not None else _sec_to_ms(_attr(related, "endpointing.delay"))

    def first_token(
        self,
        llm_first_token: float | None,
        llm_start: float | None,
        llm_ttft_reported: float | None,
        offset: float,
    ) -> float | None:
        """The LLM's first token: the event if present, else llm_start + reported ttft
        (the same token, reconstructed) so assembly/dispatch fill instead of dashing out."""
        if llm_first_token is not None:
            return llm_first_token
        if llm_start is not None and llm_ttft_reported is not None:
            return round(_to_audio(llm_start, offset) + llm_ttft_reported / 1000.0, 4)
        return None
    ...
    def first_audio(
        self, tts_first_audio: float | None, tts_start: float | None, tts_ttfb: float | None
    ) -> float | None:
        """First agent audio (end of the v2v window): the event if present, else
        tts_start + reported ttfb, else the bare tts_start."""
        if tts_first_audio is not None:
            return tts_first_audio
        if tts_start is not None and tts_ttfb is not None:
            return round(tts_start + tts_ttfb / 1000.0, 4)
        return tts_start

    # ── per-turn waterfall (shared; not an override point) ────────────────────────

    def build_turn(
        self, turn_span: Span, spans: list[Span], utterances: list[Utterance], offset: float,
        position: int = 0,
    ) -> Turn:
        ...
        # the PII transcript on the turn spans (user_turn/agent_turn), not on the stage
        # spans, so stage-scoped reads miss them — search the whole exchange for those.
        related = [s for s in spans if s.turn_id == tid]

        speech_end = self.end_of_speech(speech, offset)
        ...
        # producer-reported latency (seconds -> ms): a producer may hand ttft/ttfb as span
        ...
        endpointing = self.endpointing_ms(speech_end, stt_start, related)
        ...
        first_token = self.first_token(llm_first_token, llm_start, llm_ttft_reported, offset)
        ...
        # v2v runs the caller's end of speech -> first agent audio, and MUST share the
        # pieces' anchors or the residual goes negative.
        caller_stop = speech_end if speech_end is not None else stt_final
        agent_out = self.first_audio(tts_first_audio, tts_start, tts_ttfb)
        ...
        # any TTS segment aborted mid-synthesis => the agent's speech was truncated. The
        # turn streams sentence-by-sentence, so the cut lands on a late segment, not first.
        ...
        # Unattributed = wall-clock inside the v2v window that NO stage covered — from the
        # actual span intervals (their union), NOT by subtracting reported durations. In a
        # streaming pipeline stages overlap (the LLM still emits while TTS speaks), so
        # summing durations double-counts and would go negative; a union counts overlap once
        # and is non-negative. The endpointing hold has no span, so it is added explicitly.
...
def _turn_index(span: Span, default: int = 10**9) -> int:
    """The producer's turn.index, or `default`. Producers that don't emit it fall back to
    positional order so turns don't all collide on one index."""
