# E2E: talk to a LiveKit agent, watch Pulse pull the metrics

A minimal cascade agent (`agent.py`) that you talk to in the browser while it streams
OpenTelemetry traces to Pulse. This validates the **spans → adapter → join → metrics** path
...
## 0. Bring Pulse up (one terminal each)

```bash
# from the repo root, with the Pulse venv + a Postgres running
...
## 2. The agent (a separate venv — keep it off the Pulse package)
...
Pulse accepts the call audio as **one stereo `audio` artifact OR two mono ones**
...
3. Register both against the call (its id = the OTLP `trace_id` Pulse keyed it on):
   ```bash
...
## Notes
- The agent runs **locally**, so its OTLP exporter can reach `localhost:8000`. If Pulse runs
